...
* Datasets supplied to `jointCovariance()` / `pated()` must now contain a
...
* `data_index` defaults to 1 in every spec constructor, and a single
  data frame is auto-wrapped, so `jointCovariance(spec1(...), spec2(...),
  data = my_df)` is the shortest valid call.
...
* New testthat suite (52 tests / 245 expectations) covering every engine
  through both `jointCovariance()` and `pated()`. Tests are split into a
