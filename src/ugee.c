...
void Cgee(double *x, double *y, double *id, double *n, double *offset, int *nobs, int *p, int *parmvec, int *M_parm, double *S_beta, double *S_naivvar, double *S_robvar, double *S_score, double *S_hess, double *S_phi, double *S_R, double *tol, int *maxsz, int *S_iter, int  *silent, int *errorstate, int *scale_fix, int *compatflag)
{
/* MAIN DECLS */
...
/* cloglog case added by jh maindonald */
	...
	*errorstate = MAXITER_EXCEEDED ;
    ...
    score = VC_GEE_create_matrix((int)nclust, (int)*p, EPHEMERAL);
    hess = VC_GEE_create_matrix((int)*p, (int)*p, EPHEMERAL);
    scorebase = score->data;
    phi = 0.;
    phiLZ = 0.;
...
//scorei = VC_GEE_create_matrix((int)*p, 1, EPHEMERAL);
  scorei = VC_GEE_matmult(DRop, ei);
  scoreibase = scorei->data;
  for(j = 0; j < scorei->nrows; ++j){
    *scorebase = *scoreibase;
    scorebase++;
    scoreibase++;
  }
    ...
    *S_iter = iter;
    ...
    VC_GEE_destroy_matrix(score);
    VC_GEE_destroy_matrix(hess);
